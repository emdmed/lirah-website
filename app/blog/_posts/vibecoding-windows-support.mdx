...
**Symptom:**
The cd command appeared in the terminal but the directory did not change.
...
**Fix:**
Emit CRLF or otherwise correctly signal command submission to the Windows console. After this change, navigation mode worked.
...
**Symptom:**
Switching to file tree mode produced a warning about exceeding a file limit.
...
**Fix:**
Skip reparse points and system/hidden directories during recursion instead of increasing the file limit. After excluding junctions, file enumeration completed normally.

### 3. Incorrect Working Directory

**Symptom:**
The tool reported the working directory as `C:\Users\arsin\` instead of the project path.

**Cause:**
On Linux, a process's working directory can be read via:
...
**Fix:**
Track the working directory through process information and shell synchronization rather than reading a procfs path. Using a system information library and synchronizing the terminal prompt allowed the editor and terminal to agree on the current directory.
